...
0 0 0 20 20 20 40 40 40 60 60 60 80 80 80 100 100 100 120 120 120 140 140 140'''
    ...
    def PPM_grayscale(self):
        ...
        for rowlist in newpixellist:
            col = 0
            for pixel in rowlist:
                gray = (newpixellist[row][col][0]+newpixellist[row][col][1]+newpixellist[row][col][2])//3 # adds the rgb values and
                # divides by 3.
                newpixellist[row][col][0] = gray # the next three changes the pixel to gray.
    ...
    def PPM_flip_horizontal(self):
        ...
        :return: None
        """
        newpixellist = copy.deepcopy(self.pixellist)
        self.width = len(newpixellist[0])
        self.height = len(newpixellist)
        ...
        for rowlist in newpixellist:
            col = 0
            for pixel in rowlist:
                newpixellist[row][col][0] = newpixellist[-row][col][0] #sets the the row to flip and become in the back.
                newpixellist[row][col][1] = newpixellist[-row][col][1]
                newpixellist[row][col][2] = newpixellist[-row][col][2]
                col += 1
            row += 1
        self.PPM_updatefrompixellist(newpixellist) # This call will update all member apttributes appropriately.
        print(self.outasciifile + " output file has flipped horizontal.")
    ...
    def PPM_rotateclockwise(self):
        ...
        """
        newpixellist = self.pixellist
        cop = copy.deepcopy(newpixellist)
        flip_pic=[]
        for y in range(self.width):
            flip_pic.append([]) #append to flip_pic
            for x in range(self.height-1,-1,-1):
                flip_pic[y].append(cop[x][y])#Appends the x and y values to flip_pic
        print(self.outasciifile + " output file has turned clockwise.")
        ...
        self.PPM_updatefrompixellist(flip_pic) # This call will update all member attributes appropriately.

    # TODO FIX ME: write at least one additional PPM class method
    def PPM_make_sepia(self):
        '''
        CHanges the picture into a complete, ugly shade of sepia.
        :return:
        '''
        newpixellist = self.pixellist
        self.width = len(newpixellist[0])
        ...
        for rowlist in newpixellist:
            col = 0
            for pixel in rowlist:
                newpixellist[row][col][0] = 112 # update red to the sepia value
                newpixellist[row][col][1] = 66 # update green to sepia value
                newpixellist[row][col][2] = 20 # update blue to sepia value.
                col += 1
            row += 1
        print(self.outasciifile + " output file turned into a whole swatch of sepia")
        self.PPM_updatefrompixellist(newpixellist)


    def PPM_make_blue(self):
        """
        Colorizes current image to blue by using self.pixellist
        ...
        for rowlist in newpixellist:
            ...
            for pixel in rowlist:
                newpixellist[row][col][1] = 0 # update green
                newpixellist[row][col][0] = 0 # update red
                col += 1
            row += 1
